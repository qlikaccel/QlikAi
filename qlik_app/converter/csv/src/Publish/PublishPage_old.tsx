...
// //     // const res = await fetch("https://qlikai-new-web.onrender.com/powerbi/process", {
...
// //       // const authRes = await fetch("https://qlikai-new-web.onrender.com/powerbi/login/acquire-token", {
// //         method: "POST",
// //         headers: { "Content-Type": "application/json" },
// //         body: JSON.stringify({}),
// //       });
...
// //           // const statusRes = await fetch("https://qlikai-new-web.onrender.com/powerbi/login/status", {
// //             method: "POST",
// //           });
...
// //         // const batchRes = await fetch("https://qlikai-new-web.onrender.com/powerbi/process-batch", {
// //           method: "POST",
// //           headers: { "Content-Type": "application/json" },
// //           body: JSON.stringify({
...
// //       // const response = await fetch('https://qlikai-new-web.onrender.com/report/download-pdf', {
// //         method: 'POST',
// //         headers: { "Content-Type": "application/json" },
...
// //       // const res = await fetch("https://qlikai-new-web.onrender.com/api/migration/publish-semantic-model", {
// //         method: "POST",
// //         body: form,
// //       });
...
// //       // const res = await fetch(`https://qlikai-new-web.onrender.com/api/migration/publish-table?${params.toString()}`, {
// //         method: "POST",
// //       });
...
//         // ── BATCH PATH: multi-table → single dataset via /powerbi/process-batch ──
//         // Triggered when:
//         //   • state.batchTables is present (new flow from MultiMigrationPage), OR
//         //   • isSeparateMode + isMultiTableMode (legacy flow via sessionStorage CSVs)
...
//         // Publish all tables as a single dataset
//         let batchTables: any[] = [];

//         if (hasBatchTables) {
//           // ── New flow: full in-memory rows passed directly from MultiMigrationPage ──
//           batchTables = state.batchTables;
//           console.log(`📦 Using in-memory batch tables: ${batchTables.length} tables`);
//         } else {
//           // ── Legacy fallback: rebuild from sessionStorage CSVs ──
//           for (let i = 0; i < tableCount; i++) {
//             const csvText = state?.csvPayloads?.[`migration_csv_${i}`] || sessionStorage.getItem(`migration_csv_${i}`) || "";
//             const tName = selectedTablesToPublish[i]?.name || `Table_${i + 1}`;
//             if (!csvText) continue;
...
//             dataset_name: appName || "Qlik_Migrated_Dataset",
//             tables: batchTables,
//             // prefer_xmla: true → XMLA path for full semantic model (Open Semantic Model)
//             // only when user explicitly ticked "Separate Datasets" (maps to Premium/XMLA path)
//             prefer_xmla: exportOptions?.separateDatasets === true,
//           }),
//         });
...
//         console.log("✅ Batch publish successful:", batchData);
//         setPublishedTableName(batchData.dataset_name || appName);
//         // Prefer the Open Semantic Model deep-link when available, fall back to workspace URL
//         const batchDatasetUrl = batchData.open_semantic_model_url || batchData.workspace_url || "";
//         setDatasetURL(batchDatasetUrl);
//         setResult({
//           published_tables: batchTables.map((t: any) => ({
//             tableName: t.name,
//             rowCount: t.rows.length,
//             url: batchDatasetUrl,
//           })),
//           method: batchData.method,
//           dataset_id: batchData.dataset_id,
//           open_semantic_model: batchData.open_semantic_model ?? false,
//           deploy_message: batchData.message,
//         });
//       } else {
//         // Combined mode or single table - use updated logic (prefer navigation state payloads)
//         setCurrentStep(0);
...
//           <div className="completion-message">
//             🎉 Your dataset is now live in Power BI! You can start creating reports and dashboards with your data.
//             {result?.open_semantic_model && (
//               <span style={{ display: 'block', marginTop: '8px', color: '#107c10', fontWeight: 'bold', fontSize: '14px' }}>
//                 🧠 Open Semantic Model is enabled — you can explore the data model, edit relationships and measures directly in Power BI Service.{' '}
//                 {datasetURL && (
//                   <a href={datasetURL} target="_blank" rel="noopener noreferrer" style={{ color: '#0078d4' }}>
//                     Open Semantic Model →
//                   </a>
//                 )}
//               </span>
//             )}
//             {result?.open_semantic_model === false && result?.method && (
//               <span style={{ display: 'block', marginTop: '8px', color: '#a0522d', fontSize: '13px' }}>
//                 ℹ️ Deployed as <strong>{result.method}</strong> dataset. Open Semantic Model requires a Premium or PPU workspace.
//               </span>
//             )}
//           </div>
//         </div>
...
//               </tr>
//               <tr style={{ backgroundColor: '#f9f9f9', borderBottom: '1px solid #e0e0e0' }}>
//                 <td style={{ padding: '10px 12px', fontWeight: 'bold' }}>Deploy Method:</td>
//                 <td style={{ padding: '10px 12px', fontWeight: 'bold' }}>
//                   {result?.method === "AsAzure"
//                     ? "🧠 AsAzure (Semantic Model)"
//                     : result?.method === "PushStreaming"
//                     ? "⚡ PushStreaming"
//                     : result?.method
//                     ? result.method
//                     : "REST Push"}
...
//               </tr>
//               <tr style={{ borderBottom: '1px solid #e0e0e0' }}>
//                 <td style={{ padding: '10px 12px', fontWeight: 'bold' }}>Open Semantic Model:</td>
//                 <td style={{ padding: '10px 12px', fontWeight: 'bold', color: result?.open_semantic_model ? '#107c10' : '#a0522d' }}>
//                   {result?.open_semantic_model
//                     ? "✅ Enabled — Model View & relationships available"
//                     : "⚠️ Not available (requires Premium / PPU workspace)"}
//                 </td>
//               </tr>
//               <tr style={{ backgroundColor: '#f9f9f9', borderBottom: '1px solid #e0e0e0' }}>
//                 <td style={{ padding: '10px 12px', fontWeight: 'bold' }}>Status:</td>
  ...
  const handlePublish = async () => {
    try {
      ...
      /*if (isSeparateMode && isMultiTableMode && selectedTablesToPublish.length > 0) {
        ...
        // ── BATCH PATH: multi-table → single dataset via /powerbi/process-batch ──
        // CSV/DAX flow: selectedTablesToPublish comes from ExportPage with full in-memory data.rows
        // MultiMigrationPage flow: state.batchTables carries pre-built {name, rows} objects
        const hasBatchTables = Array.isArray(state?.batchTables) && state.batchTables.length > 0;
        ...
        if (hasBatchTables) {
          // MultiMigrationPage flow — rows already shaped correctly
          batchTables = state.batchTables;
          console.log(`📦 Using batchTables from state: ${batchTables.length} tables`);
        } else {
          // CSV/DAX flow from SummaryPage → ExportPage → PublishPage
          // selectedTablesToPublish = state.selectedTables which has data.rows in memory — use directly
          for (let i = 0; i < selectedTablesToPublish.length; i++) {
            const t = selectedTablesToPublish[i];
            const tName = t.name || `Table_${i + 1}`;
            const tableRows: any[] = t.data?.rows || [];

            if (tableRows.length > 0) {
              // ✅ Use in-memory rows directly — no CSV parsing needed, data is already correct
              batchTables.push({ name: tName, rows: tableRows });
              console.log(`📦 Table "${tName}": ${tableRows.length} rows (in-memory)`);
            } else {
              // Fallback: parse from CSV payload if rows somehow not in memory
              const csvText = state?.csvPayloads?.[`migration_csv_${i}`] || sessionStorage.getItem(`migration_csv_${i}`) || "";
              if (!csvText) { console.warn(`⚠️ No data for table "${tName}" — skipping`); continue; }

              const lines = csvText.trim().split('\n').filter((l: string) => l.trim());
              if (lines.length < 2) continue;

              // Proper quoted-field CSV parser (handles commas inside quoted values)
              ...
              const parsedRows = lines.slice(1).map((line: string) => {
                const vals = parseCSVLine(line);
                const row: any = {};
                headers.forEach((h: string, idx: number) => { row[h] = vals[idx] ?? null; });
                return row;
              });
              batchTables.push({ name: tName, rows: parsedRows });
              console.log(`📦 Table "${tName}": ${parsedRows.length} rows (CSV fallback)`);
            }
          }
        }
