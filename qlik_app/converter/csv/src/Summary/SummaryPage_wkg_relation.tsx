...
const downloadCSV = async () => {
  ...
  const handleConvertToMQuery = async () => {
    ...
    try {
      ...
      // Replace [DataSourcePath] placeholder if user has provided a path
      let finalMQuery = convertResult.m_query || "";
      if (dataSourcePath.trim()) {
        // Escape backslashes for M string literals (Windows paths use \)
        const escapedPath = dataSourcePath.trim().replace(/\\/g, "\\\\");
        finalMQuery = finalMQuery.replace(/\[DataSourcePath\]/g, `"${escapedPath}"`);
      }
