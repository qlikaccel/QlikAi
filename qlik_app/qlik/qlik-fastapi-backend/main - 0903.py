...
def call_hf_model(prompt: str):
    ...
    if resp.status_code != 200:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Hugging Face API error {resp.status_code}"
        )
...
async def generate_hf_summary(request: TableDataRequest):
    ...
    try:
        ...
        from summary_utils import generate_summary, get_data_quality_score
        ...
        metrics_str = json.dumps(metrics)[:2000]
        ...
        for col in columns[:10]:
            ...
            try:
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    col_summary += f"{col}: top values {top_vals}\n"
...
Generate exactly 7 executive summary insights from the dataset.

Rules:
- One sentence per bullet
- Start each line with "-"
- Focus on trends, anomalies, or important metrics
