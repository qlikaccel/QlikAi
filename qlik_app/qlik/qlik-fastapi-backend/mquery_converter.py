...
class MQueryConverter:
    ...
    def convert_all(
        ...
        # ── Determine which tables are referenced as RESIDENT sources ──────────
        resident_sources = set()
        for table in tables:
            if table.get("source_type") == "resident":
                src = table.get("source_path", "").strip()
                if src:
                    resident_sources.add(src)

        logger.info("[MQueryConverter] Resident sources to suppress: %s", resident_sources)

        for table in tables:
            table_name = table["name"]

            # ── Skip intermediate RESIDENT source tables ─────────────────────
            if table_name in resident_sources:
                logger.info("[MQueryConverter] Skipping intermediate table: %s", table_name)
                continue
