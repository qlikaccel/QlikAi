...
def _extract_m_produced_columns(expr: str) -> set:
    """
    Extract the set of column names that the M expression ACTUALLY PRODUCES.

    This is used to cross-validate the BIM column list against M query output
    and filter out Qlik derived columns (Date#, APPLYMAP, GROUP BY results)
    that appear in GetTablesAndKeys but are NOT in the raw CSV source file.

    Strategy: scan for explicit column names in TransformColumnTypes,
    SelectColumns, and AddColumn steps. Returns an empty set when the M
    expression uses dynamic schema (List.Transform / PromoteHeaders only) —
    in that case the caller should pass all columns through unchecked.

    Columns returned here are what Power BI will find in the rowset at
    refresh time. Declaring any other column in the BIM causes a
    "column does not exist in the rowset" error.
    """
    if not expr:
        return set()
    ...
    # Pattern 1: TransformColumnTypes with explicit named pairs
    # Skip List.Transform blocks (dynamic schema — unknown columns at parse time)
    for block_match in re.finditer(
        r"Table\.TransformColumnTypes\s*\(.*?,\s*\{(.*?)\}\s*\)",
        expr, re.DOTALL,
    ):
        block = block_match.group(1)
        if "List.Transform" in block:
            # Dynamic schema — cannot determine column names statically
            # Return empty set so caller passes all columns through
            continue
        for entry in re.finditer(
            r'\{\s*"([^"]{1,120})"\s*,\s*(?:type\s+\w+|Int64\.Type)',
            block,
        ):
            produced.add(entry.group(1).strip())

    # Pattern 2: Table.SelectColumns explicit list
    for sel_match in re.finditer(
        r"Table\.SelectColumns\s*\([^,]+,\s*\{([^}]+)\}",
        expr,
    ):
        for q in re.finditer(r'"([^"]{1,120})"', sel_match.group(1)):
            produced.add(q.group(1).strip())

    # Pattern 3: Table.AddColumn — these are columns ADDED by M steps
    # (IF conditions, derived columns). They are produced by the M expression.
    for add_match in re.finditer(
        r'Table\.AddColumn\s*\(\s*\S+\s*,\s*"([^"]{1,120})"',
        expr,
    ):
        produced.add(add_match.group(1).strip())

    # Pattern 4: Table.ExpandTableColumn explicit column list
    for exp_match in re.finditer(
        r'Table\.ExpandTableColumn\s*\([^{]+\{([^}]+)\}',
        expr,
    ):
        for q in re.finditer(r'"([^"]{1,120})"', exp_match.group(1)):
            produced.add(q.group(1).strip())

    # Remove known M step names that aren't real columns
    _step_noise = {
        "Name", "Folder Path", "Content", "Date modified", "Attributes",
        "Hidden", "Extension", "Date accessed", "Date created",
        "Source.Name", "Source.Extension",
    }
    produced -= _step_noise
    ...
    def _build_bim(
        ...
        for t in tables_m:
            ...
            if resolved_cols:
                ...
                # ── CRITICAL FIX: Cross-validate BIM columns against M expression output ──
                # resolve_output_columns may return Qlik's COMPUTED output columns (from
                # GetTablesAndKeys), which includes derived columns like full_date, total_hours,
                # emp_department_name etc. that are produced by Qlik expressions (Date#, APPLYMAP,
                # GROUP BY). These columns do NOT exist in the raw CSV file. If we declare them
                # in the BIM, Power BI tries to find them in the rowset → crash:
                #   "The '<oii>full_date</oii>' column does not exist in the rowset."
                #
                # Safe strategy: only include in the BIM columns that the M expression
                # actually produces (found via AddColumn / TransformColumnTypes / SelectColumns
                ...
                for c in resolved_cols:
                    ...
                    # If M expression produces known columns, filter to that set.
                ...
                if m_produced_cols and columns:
                    logger.info(
        ...
        # ── Relationships ─────────────────────────────────────────────────────
        # Build a set of {table_name: {col_name, ...}} from the published BIM tables
        # so we can validate that relationship columns actually exist.
        published_cols: Dict[str, set] = {}
        for tbl in tmd_tables:
            published_cols[tbl["name"]] = {c["name"] for c in tbl.get("columns", [])}
        ...
        for r in relationships:
            ...
            # Validate that both relationship columns exist in their respective BIM tables.
            # A relationship that references a column not in the BIM causes Fabric to reject
            # the entire model — even if the column will appear after first refresh.
            ft_cols = published_cols.get(ft, set())
            tt_cols = published_cols.get(tt, set())

            if ft_cols and fc not in ft_cols:
                logger.warning(
                    "[BIM] Relationship SKIPPED: '%s.%s' → '%s.%s' — "
                    "column '%s' not in BIM for table '%s' (has %d cols). "
                    "Re-publish after first dataset refresh to add this relationship.",
                    ft, fc, tt, tc, fc, ft, len(ft_cols)
                )
                continue

            if tt_cols and tc not in tt_cols:
                logger.warning(
                    "[BIM] Relationship SKIPPED: '%s.%s' → '%s.%s' — "
                    "column '%s' not in BIM for table '%s' (has %d cols). "
                    "Re-publish after first dataset refresh to add this relationship.",
                    ft, fc, tt, tc, tc, tt, len(tt_cols)
                )
                continue
        ...
        if tmd_rels:
            logger.info("[BIM] Publishing %d relationship(s)", len(tmd_rels))
        else:
            logger.warning(
                "[BIM] No relationships published. This is expected on first publish "
                "when tables have dynamic schema — re-publish after first dataset refresh."
            )
